...
function init() {
    // registerServiceWorker(); // Временно отключено для тестирования
    checkAuth();
    setupEventListeners();
}
...
function initMessagesVirtualScroller() {
    ...
    // Мгновенный скролл вниз после загрузки
...
function renderMessageItem(msg, index) {
    ...
    div.innerHTML = `
        ...
        <div class="message-time" style="
            ...
            align-self: flex-end;
            white-space: nowrap;
        ">${escapeHtml(msg.sender)} • ${escapeHtml(msg.created_at)}${statusIcon}</div>
    `;
