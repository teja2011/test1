...
# CORS с правильной конфигурацией для мобильных устройств
CORS(app, 
...
class Message(Base):
    ...
    waveform = Column(String, nullable=True)  # Waveform данные для голосовых сообщений (JSON массив)
...
def api_register():
    try:
        ...
        if not data:
            print(f"=== РЕГИСТРАЦИЯ ===")
            print("Ошибка: request.json пустой")
    ...
    try:
        ...
        resp.set_cookie('user_id', str(user.id), max_age=60*60*24*30, samesite='None', secure=False, httponly=False)
...
def api_login():
    ...
    try:
        ...
        # Cookie с правильной настройками для мобильных
        resp.set_cookie('user_id', str(user.id), max_age=60*60*24*30, samesite='None', secure=False, httponly=False)
        resp.delete_cookie('username')  # Удаляем старый cookie
        return resp
    except Exception as e:
...
def api_send_file():
    ...
    print(f"[send-file] Data length: {len(request.get_data()) if request.get_data() else 0}")
    
    try:
        form_data = request.form
        files_data = request.files
    except Exception as e:
        print(f"[send-file] Error reading form/files: {e}")
        return jsonify({'success': False, 'message': 'Invalid request format'}), 400
    
    print(f"[send-file] Form keys: {list(form_data.keys())}")
    print(f"[send-file] Files keys: {list(files_data.keys()) if files_data else 'EMPTY'}")
    ...
    file_data = form_data.get('file_data')  # Base64 данные
    file_type = form_data.get('file_type')  # 'image', 'file', 'voice', или 'video_circle'
    duration = form_data.get('duration')  # Длительность для видео-кружочков
    waveform = form_data.get('waveform')  # Waveform данные для голосовых сообщений

    # Поддержка JSON запроса (для видео-кружочков)
    try:
        json_data = request.get_json(silent=True)
    except Exception as e:
        print(f"[send-file] Error parsing JSON: {e}")
        json_data = None
    ...
    if json_data:
        ...
        print(f"[send-file] JSON content length: {len(json_data.get('content', '')) if json_data.get('content') else 0}")
        ...
        print(f"[send-file] WARNING: No JSON data found")
        ...
        # Обработка голосовых сообщений через multipart/form-data
            ...
            if file and file.filename:
                try:
                    ...
                    content = upload_result['secure_url']
                    # Получаем длительность из метаданных Cloudinary или из FormData
                    if 'duration' in upload_result:
                        msg_duration = str(int(upload_result['duration'])) + 's'
                    elif duration:
                        msg_duration = duration
                        ...
                        msg_duration = '0s'
                    print(f"[send-file] Voice uploaded to Cloudinary: {content}, duration: {msg_duration}")
                except Exception as cloudinary_error:
                    ...
                    content = f'data:{mime_type};base64,{file_base64}'
                    # Устанавливаем длительность из FormData или по умолчанию
                    msg_duration = duration if duration else '0s'
                    print(f"[send-file] Voice converted to base64, length: {len(content)}, duration: {msg_duration}")
            else:
                return jsonify({'success': False, 'message': 'No file uploaded'})
