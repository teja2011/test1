from flask import Flask, render_template_string, request, jsonify, redirect, make_response, send_from_directory, stream_with_context
...
app.config['COMPRESS_MIN_SIZE'] = 500  # Сжимать ответы от 500 байт
app.config['COMPRESS_LEVEL'] = 6  # Уровень сжатия (1-9)
Compress(app)

# === Оптимизация: ограничение размера файлов (10MB) ===
app.config['MAX_CONTENT_LENGTH'] = 10 * 1024 * 1024  # 10MB max

# === Оптимизация: chunked transfer encoding ===
app.config['SEND_FILE_MAX_AGE_DEFAULT'] = 300  # 5 минут кэширование

# === WebSocket для статуса отправки файлов ===
...
def api_login():
    ...
    try:
        ...
        # Конвертируем last_seen в МСК для клиента
        last_seen_msk = to_msk(user.last_seen) if user.last_seen else None
        resp = make_response(jsonify({'success': True, 'user': {
            'id': user.id,
            'username': user.username,
            'avatar_color': user.avatar_color or '6366f1',
            'avatar_url': user.avatar_url,
            'jt_username': user.jt_username,
            'last_seen': last_seen_msk.isoformat() if last_seen_msk else None
        }}))
        resp.set_cookie('user_id', str(user.id), max_age=60*60*24*30, samesite='lax')
...
def api_send():
    ...
    try:
        ...
        sender = db.query(User).filter_by(id=user.id).first()
        message_data = {
            'id': msg_id,
            'sender': sender.username if sender else 'Unknown',
            'content': content,
        ...
        if recipient_id:
            ...
            if recipient:
                ...
                print(f"[WS] Message {msg_id} sent to user_{recipient_id}")
...
def api_send_file():
    ...
    try:
        ...
        if recipient_id:
            try:
                ...
                if recipient:
                    ...
                    # WebSocket уведомление о статусе
        ...
        # WebSocket уведомление отправителю
        socketio.emit('file_status', {
            'status': 'sent',
            'message_id': msg.id,
    ...
    try:
        ...
        if CLOUDINARY_CONFIGURED:
            ...
            try:
                ...
                # Используем Cloudinary URL с трансформацией для квадратного аватара
                # Заменяем /upload/ на /upload/w_200,h_200,c_fill,g_face/ для кропа
...
# ============================================
# WEBSOCKET: Статус отправки файлов
# ============================================
@socketio.on('connect')
def ws_connect():
    ...
    if user:
        ...
        print(f"[WS] User {user.id} connected")
...
def ws_disconnect():
    ...
    user = get_current_user()
    if user:
        print(f"[WS] User {user.id} disconnected")

@socketio.on('subscribe')
def ws_subscribe(data):
    """Подписка на уведомления по user_id"""
    from flask_socketio import join_room
    user_id = data.get('user_id')
    if user_id:
        join_room(f'user_{user_id}')
        print(f"[WS] Subscribed to user_{user_id}")
...
# ============================================
# VIDEO STREAMING: Прогрессивная загрузка
# ============================================
@app.route('/api/video/<int:message_id>')
def stream_video(message_id):
    """Потоковая отдача видео с поддержкой Range запросов"""
    user = get_current_user()
    if not user:
    ...
    try:
        ...
        if not msg or msg.file_type != 'video':
        ...
        # Поддержка Range запросов для прогрессивной загрузки
        ...
        if range_header:
            ...
            if range_match:
                ...
                end = int(range_match.group(2)) if range_match.group(2) else video_size - 1
            ...
            response = make_response(video_bytes[start:end + 1])
            response.status_code = 206
            response.headers['Content-Range'] = f'bytes {start}-{end}/{video_size}'
            response.headers['Content-Length'] = str(end - start + 1)
        else:
            # Отправляем всё видео
...
if __name__ == '__main__':
    ...
    # Для разработки запускаем с socketio
    # Для продакшена используйте: gunicorn -c gunicorn_config.py messenger_server:app
