...
try:
    ...
    if (os.environ.get('CLOUDINARY_CLOUD_NAME') and 
        ...
        print("[Cloudinary] Environment variables not set")
...
def api_upload_avatar():
    ...
    if CLOUDINARY_CONFIGURED or os.environ.get('CLOUDINARY_CLOUD_NAME'):
        ...
        try:
            ...
            avatar_url = upload_result['secure_url'].replace('/upload/', '//upload/w_200,h_200,c_fill,g_face/')
        ...
        # Локально - сохраняем в папку (только для локальной разработки)
        print(f"[Avatar] Saving locally (no Cloudinary configured)...")
