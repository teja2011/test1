...
def check_and_create_tables():
    ...
    try:
        with engine.connect() as conn:
            ...
            # Проверяем и добавляем колонку last_seen если её нет
            try:
                if DATABASE_URL:
                    # PostgreSQL
                    ...
                    """)).fetchone()
                    if not col_check[0]:
                        print("Добавляю колонку last_seen...")
...
def api_me():
    user = get_current_user()
    if user:
        # Обновляем last_seen при загрузке страницы
        db = get_db()
        try:
...
def api_users():
    ...
    try:
        ...
        now = datetime.now()
        for u in users:
            # Считаем онлайн если last_seen был в последние 60 секунд
            is_online = False
            last_seen_str = None
            if u.last_seen:
                last_seen_str = u.last_seen.strftime('%H:%M')
...
def serve_static_file(filename):
    ...
    if filename.startswith('api/') or filename.startswith('chat'):
