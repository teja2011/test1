...
class User(Base):
    ...
    bio = Column(String(200), nullable=True)  # Описание пользователя
...
def api_me():
    ...
    if user:
        ...
        return jsonify({'id': user.id, 'username': user.username, 'avatar_color': user.avatar_color or '6366f1', 'avatar_url': user.avatar_url, 'jt_username': user.jt_username, 'bio': user.bio})
...
@app.route('/api/bio/set', methods=['POST'])
def api_bio_set():
    """Установка bio (описания) для пользователя"""
    user_id = request.cookies.get('user_id')
    if not user_id:
        return jsonify({'success': False, 'message': 'Not authorized'})
    ...
    try:
        ...
        # Проверка: макс 200 символов
        if len(bio) > 200:
            return jsonify({'success': False, 'message': 'Описание должно быть не более 200 символов'})

        user.bio = bio if bio else None
        db.commit()
        return jsonify({'success': True, 'bio': user.bio})
    except Exception as e:
        db.rollback()
