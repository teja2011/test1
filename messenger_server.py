...
# Гарантируем создание таблиц перед первым запросом
@app.before_request
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--reset-db':
        ...
        reset_db()
        print("\nБаза данных сброшена. Запускайте сервер без --reset-db")
        sys.exit(0)
    
    print("=== Инициализация базы данных ===")
    # Создаём таблицы гарантированно перед запуском
    try:
        Base.metadata.create_all(engine)
        print("Таблицы созданы: users, messages, notifications")
    except Exception as e:
        print(f"Ошибка создания таблиц: {e}")
        print("Проверьте DATABASE_URL и подключение к БД")
        sys.exit(1)
