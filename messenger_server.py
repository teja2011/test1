...
if DATABASE_URL:
    # Удаляем channel_binding из URL если есть
    DATABASE_URL = DATABASE_URL.replace('&channel_binding=require', '').replace('&channel_binding=prefer', '')
    # Для psycopg2 передаём sslmode отдельно
    if 'sslmode=require' in DATABASE_URL:
        DATABASE_URL = DATABASE_URL.replace('?sslmode=require', '').replace('&sslmode=require', '')
        engine = create_engine(DATABASE_URL, echo=False, pool_pre_ping=True, connect_args={'sslmode': 'require'})
    else:
...
# CORS настройка с явными заголовками для iOS Safari
@app.after_request
def after_request(response):
    # Разрешаем credentials для всех источников
    response.headers.add('Access-Control-Allow-Credentials', 'true')
    response.headers.add('Access-Control-Allow-Origin', request.headers.get('Origin', '*'))
    response.headers.add('Access-Control-Allow-Headers', 'Content-Type,Authorization')
    response.headers.add('Access-Control-Allow-Methods', 'GET,PUT,POST,DELETE,OPTIONS')
    # Важно для iOS Safari - явно указываем Vary
    response.headers.add('Vary', 'Origin')
    return response

CORS(app, supports_credentials=True, origins='*', allow_headers=['Content-Type', 'Authorization'], methods=['GET', 'PUT', 'POST', 'DELETE', 'OPTIONS'])
...
def get_current_user():
    ...
    print(f"[get_current_user] Cookie user_id: {user_id}")
    if not user_id:
        print(f"[get_current_user] No user_id cookie")
        return None
    db = get_db()
    try:
        user = db.query(User).filter_by(id=int(user_id)).first()
        if user:
            print(f"[get_current_user] User found: id={user.id}, username={user.username}")
        else:
...
def api_me():
    user_id = request.cookies.get('user_id')
    print(f"[API /me] Cookie user_id: {user_id}")
    print(f"[API /me] User-Agent: {request.headers.get('User-Agent', 'Unknown')}")
    
    user = get_current_user()
    if user:
        print(f"[API /me] User found: id={user.id}, username={user.username}, avatar_url={user.avatar_url}, jt_username={user.jt_username}")
    ...
    print(f"[API /me] No user (cookie: {user_id})")
    return jsonify(None)
...
def api_register():
    ...
    try:
        ...
        # Определяем HTTPS и устанавливаем secure соответственно
        is_https = request.headers.get('X-Forwarded-Proto', 'http') == 'https' or request.is_secure
        print(f"[Register] Cookie settings: secure={is_https}, samesite='lax', httponly=False")
        ...
        print(f"[Register] Cookie установлен: user_id={user.id}")
        return resp
    except Exception as e:
        db.rollback()
...
def api_login():
    ...
    try:
        ...
        # Определяем HTTPS и устанавливаем secure соответственно
        is_https = request.headers.get('X-Forwarded-Proto', 'http') == 'https' or request.is_secure
        print(f"Cookie settings: secure={is_https}, samesite='lax', httponly=False")
        
        resp.set_cookie('user_id', str(user.id), 
                       max_age=60*60*24*30, 
                       samesite='lax', 
                       secure=is_https,  # True для HTTPS (Vercel), False для HTTP
                       httponly=False)
        resp.delete_cookie('username')  # Удаляем старый cookie
        
        print(f"Cookie установлен: user_id={user.id}")
...
def serve_avatar(filename):
    """Раздача аватарок"""
    import os
    # В serverless-среде аватарки в /tmp, иначе - локально
    if os.environ.get('VERCEL') or os.environ.get('AWS_LAMBDA_FUNCTION_NAME'):
