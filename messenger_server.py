...
def get_current_user():
    ...
    db = get_db()
    try:
        result = db.execute("SELECT * FROM users WHERE id = ?", (int(user_id),))
        row = result.fetchone()
        db.close()
        if row:
            return {
...
def api_register():
    ...
    try:
        ...
        import traceback
        traceback.print_exc()
        return jsonify({'success': False, 'message': 'Ошибка регистрации: ' + str(e)})
...
def api_messages(recipient_id=None):
    ...
    try:
        ...
        for row in rows:
            ...
            res = db.execute("SELECT username FROM users WHERE id = ?", (m['sender_id'],))
            sender_row = res.fetchone()
...
def api_send():
    ...
    try:
        ...
        if recipient_id:
            result = db.execute("SELECT username FROM users WHERE id = ?", (recipient_id,))
            recipient_row = result.fetchone()
...
def api_send_file():
    ...
    try:
        ...
        if recipient_id:
            try:
                result = db.execute("SELECT username FROM users WHERE id = ?", (int(recipient_id),))
                recipient_row = result.fetchone()
                if recipient_row:
                    create_notification(
                        db=db,
...
def api_notifications():
    ...
    try:
        ...
        for n in notifications:
            sender = None
            if n[1]:  # sender_id
                sender_result = db.execute("SELECT id, username, avatar_color FROM users WHERE id = ?", (n[1],))
...
def api_notifications_unread():
    ...
    db = get_db()
    try:
        result = db.execute("SELECT COUNT(*) FROM notifications WHERE user_id = ? AND is_read = 0", (user['id'],))
...
def api_notifications_mark_single_read(notification_id):
    ...
    db = get_db()
    try:
        result = db.execute("SELECT id FROM notifications WHERE id = ? AND user_id = ?", (notification_id, user['id']))
...
def api_change_username():
    ...
    try:
        ...
        result = db.execute("SELECT id FROM users WHERE username = ?", (new_username,))
        existing = result.fetchone()
...
def api_last_messages():
    ...
    try:
        ...
        for partner_id, msg in last_messages.items():
            sender_result = db.execute("SELECT username FROM users WHERE id = ?", (msg[1],))
            sender_row = sender_result.fetchone()
...
def api_username_set():
    ...
    db = get_db()
    try:
        result = db.execute("SELECT id FROM users WHERE id = ?", (int(user_id),))
        ...
        result = db.execute("SELECT id FROM users WHERE jt_username = ? AND id != ?", (jt_username, int(user_id)))
        existing = result.fetchone()
        if existing:
...
def api_upload_avatar():
    ...
    try:
        ...
        result = db.execute("SELECT id, username, avatar_url FROM users WHERE id = ?", (int(user_id),))
        user_row = result.fetchone()
        if not user_row:
