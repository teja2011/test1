...
def add_tg_username_column_if_not_exists():
    """Добавить колонку tg_username если её нет (вызывать ДО любых запросов к users)"""
    try:
        with engine.connect() as conn:
            if DATABASE_URL:
                ...
                if table_exists:
                    ...
                    """)).fetchone()
                    if not col_check[0]:
                        print("Добавляю колонку tg_username в PostgreSQL...")
                        conn.execute(text("ALTER TABLE users ADD COLUMN tg_username VARCHAR(50);"))
                        conn.commit()
                        print("Колонка tg_username добавлена")
            else:
                # SQLite
                table_check = conn.execute(text("""
                ...
                if table_exists:
                    col_check = conn.execute(text("""
                        ...
                        WHERE name = 'tg_username';
                    """)).fetchone()
                    if not col_check:
                        print("Добавляю колонку tg_username в SQLite...")
                        conn.execute(text("ALTER TABLE users ADD COLUMN tg_username VARCHAR(50);"))
                        conn.commit()
                        print("Колонка tg_username добавлена")
    except Exception as e:
        print(f"Ошибка при добавлении колонки tg_username: {e}")
...
class User(Base):
    ...
    tg_username = Column(String(50), unique=True, nullable=True)  # @username как в Telegram
...
def init_tables():
    ...
    try:
        ...
        # Добавляем колонку tg_username если её нет (после создания таблиц)
        add_tg_username_column_if_not_exists()
...
# ============================================
# API ДЛЯ РАБОТЫ С @USERNAME (TELEGRAM-STYLE)
# ============================================

import re

def validate_tg_username(username):
    """
...
def api_check_username():
    ...
    # Валидация формата
    is_valid, message = validate_tg_username(username)
    if not is_valid:
        return jsonify({
            'available': False,
            'valid': False,
    ...
    try:
        existing = db.query(User).filter(
            User.tg_username.ilike(username)
        ).first()
...
def api_set_username():
    ...
    try:
        ...
        user.tg_username = username.lower()
        db.commit()
        
        return jsonify({
            'success': True,
            'username': user.tg_username,
            'message': f'Username @{user.tg_username} установлен'
        })
    except Exception as e:
        db.rollback()
        return jsonify({
...
def api_search_username():
    ...
    try:
        ...
        users = db.query(User).filter(
            User.tg_username.ilike(f'{query}%')
        ).limit(10).all()
