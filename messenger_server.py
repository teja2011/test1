...
if __name__ == '__main__':
    ...
    app.run(host='0.0.0.0', port=5000, debug=False)from flask import Flask, render_template_string, request, jsonify, redirect, make_response, send_from_directory
...
def api_users():
    ...
    # Обновляем last_seen текущего пользователя
    db = get_db()
    try:
        user.last_seen = datetime.now(timezone.utc)
        db.commit()
            ...
            try:
                if u.last_seen:
                    ...
                    if last_seen_date >= today:
                        last_seen = 'сегодня в ' + last_seen_moscow.strftime('%H:%M')
                    else:
                        last_seen = last_seen_moscow.strftime('%d.%m.%Y в %H:%M')
